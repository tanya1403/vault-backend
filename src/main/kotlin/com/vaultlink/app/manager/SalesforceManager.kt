...
Branch_Name__r.Branch_Address_line_2__c, 
    ...
    fun fetchPickupRequestById(recordId: String): PickupRequest? {
        val query =
            """
                SELECT 
                Id,
                ...
                Expected_Pickup_Date__c,
                Actual_Pickup_Date__c
                FROM Documents_Pickup__c WHERE Id = '$recordId'
                """.trimIndent()

        val response = sfConnection.get(query) ?: return null
        val records = response.optJSONArray("records") ?: return null
    ...
    private fun updateDocumentSentToKleetoDate(branchId: String, sentDate: String) {
        ...
        // Salesforce Composite API has a limit of 200 records per call
